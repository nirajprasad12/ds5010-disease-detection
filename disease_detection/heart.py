...
resource_path = '/'.join(('datasets', 'heart.csv'))  
template = pkg_resources.resource_stream(resource_package, resource_path)

dataset = pd.read_csv(template)

X = pd.DataFrame(dataset.drop(['target'], axis=1).values)
Y = dataset['target']
...
class HeartDisease:
    def __init__(self, X_inp):
        ...
        if X_inp is not None:
            for i in X_inp:
                if len(i) != len(X.columns):
                    self.flag = 1
                    raise ValueError("Input array length must be {} for all rows".format(len(X.columns)))
